...
import { retrieveDistrictCategory } from "../../actions/apiadd";
...
class DistrictCategory extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      value: {
  ...
  submitData = (e) => {
    ...
    if (Object.keys(errors).length === 0) {
      ...
      if (check.includes(false)) {
        ...
        fetch("http://5.9.111.198:13880/api/DistrictCategories", {
          method: "POST",
          headers: {
  ...
  editForm = (id) => {
    ...
    axios
      .get(`http://5.9.111.198:13880/api/DistrictCategories/${id}`)
      .then((res) => {
        ...
        <div>
          <CRow>
            <CCol xs="12" sm="12">
              <CCard>
                <CCardHeader>
                  <h4>District Category List</h4>
                </CCardHeader>
                <CCardBody>
                  <Table
                    dataSource={this.props.districtCategoryList.sort((a, b) =>
                      a.id < b.id ? 1 : -1
                    )}
                    rowKey={"id"}
                  >
...
export default connect(mapStateToProps, mapDispatchToProps)(DistrictCategory);
