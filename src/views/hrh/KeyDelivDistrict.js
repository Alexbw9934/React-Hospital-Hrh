...
import KeyCsvDistrict from "./KeyCsvDistrict";
const { confirm } = Modal;
const config = localStorage.getItem("token");
class KeyDelivDistrict extends Component {
  constructor() {
    super();
    this.state = {
      rowsData: {},
  ...
  componentDidMount() {
    ...
    if (this.props.history.location.state.data) {
      ...
      axios({
        url: `${process.env.REACT_APP_API_URL}StateDistributionKeyDeliverables/${paramId}`,
        method: "GET",
      }).then((response) => {
        this.setState({ rowsData: response.data });
      });
    }
    if (this.props.history.location.state.kId) {
  ...
  handleSubmit = (e) => {
    ...
    if (
      ...
      this.state.keydeil.map((data) => {
        ...
        fetch(
          `${process.env.REACT_APP_API_URL}DistrictDistributionKeyDeliverables`,
          {
            method: "POST",
            headers: {
  ...
  render() {
    const { errors, csvOrmanually, rowsData } = this.state;
    const propState = this.props.history.location.state;
    let rowsAllocate =
      this.state.keydeil.length > 0
        ? this.state.keydeil.reduce((prev, cur) => {
            let val = {};
            val = {
              regSanctioned:
                Number(prev.regSanctioned) + Number(cur.regSanctioned),
              regInPlace: Number(prev.regInPlace) + Number(cur.regInPlace),
              contractualApprov:
                Number(prev.contractualApprov) + Number(cur.contractualApprov),
              contractualInPlace:
                Number(prev.contractualInPlace) +
                Number(cur.contractualInPlace),
              regContractSanctioned:
                Number(prev.regContractSanctioned) +
                Number(cur.regContractSanctioned),
              regContractInPlace:
                Number(prev.regContractInPlace) +
                Number(cur.regContractInPlace),
            };
            return val;
          })
        : {};
...
export default connect(mapStateToProps, mapDispatchToProps)(KeyDelivDistrict);
