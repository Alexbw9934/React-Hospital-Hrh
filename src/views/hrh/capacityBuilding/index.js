...
export class CapacityBuildingStatusDetails extends Component {
  constructor() {
    super();
    this.state = {
      country: "",
      ...
      columnsNational: [
        ...
        },
        {
          ...
          render: (text, record) => (
            <Link
              to={{
                pathname: `/hrh/CapacityBuildingStatusDetails`,
  ...
  componentDidMount() {
    ...
    axios({
      url: `${process.env.REACT_APP_API_URL}RegularCadreSDs`,
      method: "GET",
    }).then((response) => {
  ...
  newCollector = () => {
    if (this.state.role === "admin_role") {
      this.props.history.push({
        pathname: "/hrh/CapacityBuildingStatusNational",
      });
    } else {
      this.props.history.push({
        pathname: "/hrh/CapacityBuildingStatus",
      });
    }
  };
  render() {
          ...
          <CCol xs="12" sm="12">
            <CCard>
              <CCardHeader>
                ...
                >
                  {role === "admin_role" ? (
                    <h4>Capacity Building Status Report-National</h4>
                  ) : role === "state_role" ? (
                    <h4>Capacity Building Status Report - State</h4>
                  ) : (
                    <h4>Capacity Building Status Report - District</h4>
                  )}
                </div>
              </CCardHeader>
              <CCard>
