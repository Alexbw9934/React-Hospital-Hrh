...
export class AmendReport extends Component {
  constructor() {
    super();
    this.state = {
      country: "",
  ...
  componentDidMount() {
    ...
    if (this.state.role === "district_role") {
      axios({
        ...
        let arr=response.data.filter(data=>data.districtStatus===5)
    ...
    if (this.state.role === "state_role") {
        axios({
          ...
          let arr=response.data.filter(data=>data.stateStatus===5)
...
export default connect(mapStateToProps, mapDispatchToProps)(AmendReport);
