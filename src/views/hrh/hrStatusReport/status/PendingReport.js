...
export class PendingReport extends Component {
  constructor() {
    super();
    this.state = {
      country: "",
      ...
      columns: [
        ...
        },
        {
          ...
          key: "districtStatus",
          render: (text) => <a>Pending Approval</a>,
        },
        {
          title: "Action",
          key: "action",
  ...
  componentDidMount() {
    ...
    if (this.state.role == "district_role") {
      axios({
        ...
        let arr=response.data.filter(data=>data.districtStatus==1)
    ...
    if (this.state.role == "state_role") {
      axios({
        ...
        let arr=response.data.filter(data=>data.stateStatus==1)
...
export default connect(mapStateToProps, mapDispatchToProps)(PendingReport);
