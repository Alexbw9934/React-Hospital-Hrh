...
export class DraftReport extends Component {
  ...
  componentDidMount() {
    ...
    if (this.state.role == "district_role") {
      axios({
        ...
        let arr=response.data.filter(data=>data.districtStatus==0)
    ...
    if (this.state.role == "state_role") {
        axios({
          ...
          let arr=response.data.filter(data=>data.stateStatus==0)
  ...
  StoreCollector = (e,id) => {
    if (this.state.role == "admin_role") {
        ...
        this.props.history.push({
          pathname:
            "/hrh/HRStatusReportDetails/HRStatusDistrict",
          state: {
            data: id,
          },
        });
...
export default connect(mapStateToProps, mapDispatchToProps)(DraftReport);
