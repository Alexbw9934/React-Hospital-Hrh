...
export class CancelReport extends Component {
  constructor() {
    super();
    this.state = {
      country: "",
  ...
  componentDidMount() {
    ...
    if (this.state.role === "district_role") {
      axios({
        ...
        let arr=response.data.filter(data=>data.districtStatus===6)
    ...
    if (this.state.role === "state_role") {
        axios({
          ...
          let arr=response.data.filter(data=>data.stateStatus===6)
...
export default connect(mapStateToProps, mapDispatchToProps)(CancelReport);
