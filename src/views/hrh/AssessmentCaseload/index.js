...
export class AssessmentCaseloadDetails extends Component {
  constructor() {
    super();
    this.state = {
      country: "",
      ...
      columnsNational: [
        ...
        },
        {
          ...
          key: "action",
          render: (text, record) => (
              <Link to={{pathname: `/hrh/AssessmentCaseload`, state: { data: record.id}}}>
              <CButton
                style={{ backgroundColor: "blue" }}
                // onClick={(e) => this.StoreCollector(e, record.id)}
              >
  ...
  newCollector = () => {
   ...
   { this.props.history.push({
      pathname: "/hrh/AssessmentCaseload",
    });}
    else{
        this.props.history.push({
            pathname: "/hrh/AssessmentCaseload",
