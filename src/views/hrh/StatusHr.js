...
class StatusHr extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      loading: true,
  ...
  componentDidMount() {
    axios({
      ...
      this.setState({ StatusHrs: response.data });
    });
    axios({
  ...
  StoreCollector = (id) => {
    this.props.history.push("/hrh/AddStatusHr", { statusHrId: id });
  };
  render() {
    return (
      <div>
        <CRow>
          <CCol xs="12" sm="12">
            <CCard>
              ...
              >
                <h4>
                  Status<small>HRs</small>
                </h4>
                <CButton
                  style={{ backgroundColor: "blue" }}
                  onClick={() => this.props.history.push("/hrh/AddStatusHr")}
                >
                  <b style={{ color: "white" }}>Add Status Hr</b>
                </CButton>
              </CCardHeader>
              <CCardBody>
...
export default withRouter(StatusHr);
