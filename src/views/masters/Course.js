...
import { retrieveCourse, retrieveTypeOfCourse } from "../../actions/apiadd";
...
class Course extends React.Component {
  ...
  validate = () => {
    ...
    if (value.typeofCourseId == 0)
      errors.typeofCourseId = "Select the Type of Course.";
    return errors;
  };
  submitData = (e) => {
    e.preventDefault();
    ...
    if (Object.keys(errors).length == 0) {
      ...
      let check = this.props.courseList.map((data) => {
      ...
      if (check.includes(false)) {
        ...
        fetch("http://5.9.111.198:13880/api/Courses", {
          method: "POST",
          headers: {
  ...
  editForm = (id) => {
    ...
    axios.get(`http://5.9.111.198:13880/api/Courses/${id}`).then((res) => {
  ...
  updateValidate = () => {
    ...
    if (this.state.editValue.typeofCourseId == 0)
      errors.typeofCourseId = "Select the Type of Course.";
    return errors;
  };
  updateData = (e, id) => {
    ...
    if (Object.keys(errors).length == 0) {
      ...
      fetch(`http://5.9.111.198:13880/api/Courses/${id}`, {
        method: "PUT",
        headers: {
          Accept: "application/json",
          "Content-Type": "application/json",
              ...
              <CCardBody>
                ...
                {this.state.edit ? (
                  ...
                  >
                    <CFormGroup row>
                      <CCol md="4">
                        <CLabel htmlFor="text-input">Course</CLabel>
                      </CCol>
                      <CCol xs="12" sm="12">
                        <CInput
                          id="name"
                    ...
                    <CFormGroup row>
                      <CCol md="4">
                        <CLabel htmlFor="text-input">Type of Course</CLabel>
                      </CCol>
                      <CCol xs="12" sm="12">
                        <CSelect
                          id="typeofCourseId"
                          ...
                          placeholder="Select"
                          onChange={this.updateChange}
                          value={editValue.typeofCourseId || ""}
                          invalid={errors.typeofCourseId ? true : false}
                        >
                          <option value="0">-Select-</option>
                          {this.props.typeofCourseList &&
                            this.props.typeofCourseList.map((user) => (
                              <option key={user.id} value={user.id}>
                                {user.name}
                              </option>
                            ))}
                        </CSelect>
                        <FormFeedback>{errors.typeofCourseId}</FormFeedback>
                      </CCol>
                    </CFormGroup>
                    <CFormGroup row>
                      <CCol md="4">
                    ...
                    <CFormGroup row>
                      ...
                      <CCol xs="12" sm="12">
                        <CSelect
                          ...
                          placeholder="Select"
                          onChange={this.handleChange}
                          value={this.state.value.typeofCourseId || ""}
                          invalid={errors.typeofCourseId ? true : false}
                        >
                          <option value="0">-Select-</option>
                          {this.props.typeofCourseList &&
          ...
          <CRow>
            <CCol xs="12" sm="12">
              <CCard>
                ...
                <CCardBody>
                  <Table
                    dataSource={this.props.courseList.sort((a, b) =>
                      a.id < b.id ? 1 : -1
                    )}
                    rowKey={"id"}
                  >
                    ...
                    <Table.Column
                      ...
                      dataIndex="typeofCourseId"
                      key="typeofCourseId"
                      render={(text, user) => (
                        <>
                          {this.props.typeofCourseList
                            .filter((data) => data.id == user.typeofCourseId)
                            .map((id) => {
                              return id.name;
                            })}
                        </>
...
const mapStateToProps = (state) => {
  return {
    courseList: state.apiadd.courseList,
    typeofCourseList: state.apiadd.typeofCourseList,
  };
};
