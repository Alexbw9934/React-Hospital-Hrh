...
import { retrieveReservationCategory } from "../../actions/apiadd";
...
class ReservationCategory extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      value: {
  ...
  componentDidMount() {
    this.props.retrieveReservationCategory();
  }
  ...
  submitData = (e) => {
    ...
    if (Object.keys(errors).length == 0) {
      ...
      if (check.includes(false)) {
        ...
        fetch("http://5.9.111.198:13880/api/ReservationCategories", {
          method: "POST",
          headers: {
  ...
  editForm = (id) => {
    ...
    axios
      .get(`http://5.9.111.198:13880/api/ReservationCategories/${id}`)
      .then((res) => {
        ...
        <div>
          <CRow>
            <CCol xs="12" sm="12">
              <CCard>
                <CCardHeader>
                  <h4>Reservation Category List</h4>
                </CCardHeader>
                  <CCardBody>
                    <Table
                      dataSource={this.props.reservationCategoryList.sort(
                        (a, b) => (a.id < b.id ? 1 : -1)
                      )}
                      rowKey={"id"}
                    >
...
const mapStateToProps = (state) => {
  return {
    reservationCategoryList: state.apiadd.reservationCategoryList,
  };
};
const mapDispatchToProps = (dispatch) =>
  bindActionCreators(
    {
      retrieveReservationCategory,
    },
    dispatch
  );
