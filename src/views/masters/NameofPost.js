...
import { retrieveCategoryOfPosition, retrieveProgramType } from "../../actions/apiadd";
import { bindActionCreators } from "redux";
const { confirm } = Modal;
// const config=localStorage.getItem('token')
function NameofPost(props) {
  ...
  useEffect(() => {
    ...
    dispatch(retrieveCategoryOfPosition());
  ...
  useEffect(() => {
    if (data.fmrId != 0) {
      dispatch(fmrToCat(data.fmrId));
    }
  }, [data.fmrId]);
  useEffect(() => {
    if (editValue.fmrId != 0) {
      dispatch(fmrToCat(editValue.fmrId));
    }
  }, [editValue.fmrId]);
  return (
    <div>
      <CRow>
        <CCol xs="12" sm="12">
          <CCard>
              ...
              {edit ? (
                ...
                >
                  ...
                  <CFormGroup row>
                    ...
                    <CCol xs="12" sm="12">
                      <CSelect
                        id="categoryofPostionId"
                        name="categoryofPostionId"
                        placeholder="Select"
                        onChange={updateChange}
                        value={editValue.categoryofPostionId || ""}
                      >
                        <option value="0">-Select-</option>
                        {props.fmrToCatOption &&
                          props.fmrToCatOption.map((user) => (
                            <option key={user.id} value={user.id}>
                              {user.categoryName}
                            </option>
                          ))}
                      </CSelect>
                      <FormFeedback>{errors.categoryofPostionId}</FormFeedback>
                    </CCol>
                  </CFormGroup>
                  <CFormGroup row>
                    <CCol md="4">
                  ...
                  <CFormGroup row>
                    <CCol xs="12" sm="12">
                      <Checkbox
                        onChange={(e)=>setEditValue({...editValue,showkey:e.target.checked})}
                  ...
                  <CFormGroup row>
                    ...
                    <CCol xs="12" sm="12">
                      <CSelect
                        ...
                        placeholder="Select"
                        onChange={handleChange}
                        value={data.categoryofPostionId || ""}
                      >
                        <option value="0">-Select-</option>
                        {props.fmrToCatOption &&
                          props.fmrToCatOption.map((user) => (
            ...
            <CCard>
              <CCardHeader>Name of Post List</CCardHeader>
              <CCardBody>
                <Table
                ...
                >
                  ...
                  ></Table.Column>
                    ...
                    dataIndex="categoryofPostionId"
                    key="categoryofPostionId"
                    render={(text, user) => (
                      <>
...
export default connect(mapStateToProps, mapDispatchToProps)(NameofPost);
