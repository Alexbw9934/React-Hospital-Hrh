...
import { retrieveRecruitmentConuctor } from "../../actions/apiadd";
...
class RecruitmentConuctor extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      value: {
  ...
  componentDidMount() {
    this.props.retrieveRecruitmentConuctor();
  }
  ...
  submitData = (e) => {
    ...
    if (Object.keys(errors).length === 0) {
      ...
      let check = this.props.recruitmentConductorList.map((data) => {
      ...
      if (check.includes(false)) {
        ...
        fetch("http://5.9.111.198:13880/api/RecruitmentConductors", {
          method: "POST",
          headers: {
  ...
  editForm = (id) => {
    ...
    axios
      .get(`http://5.9.111.198:13880/api/RecruitmentConductors/${id}`)
      .then((res) => {
  ...
  updateData = (e, id) => {
    ...
    if (Object.keys(errors).length === 0) {
      ...
      if (check.includes(false)) {
        ...
        fetch(`http://5.9.111.198:13880/api/RecruitmentConductors/${id}`, {
          method: "PUT",
          headers: {
            Accept: "application/json",
            "Content-Type": "application/json",
                    ...
                    <CFormGroup row>
                      ...
                      <CCol xs="12" sm="12">
                        <CInput
                          id="name"
                          name="name"
                          placeholder="Recruitment Conductor"
        ...
        <div>
          <CRow>
            <CCol xs="12" sm="12">
              <CCard>
                <CCardHeader>
                  <h4>Recruitment Conductor List</h4>
                </CCardHeader>
                <CCardBody>
                  <Table
                    dataSource={this.props.recruitmentConductorList.sort(
                      (a, b) => (a.id < b.id ? 1 : -1)
                    )}
                    rowKey={"id"}
                  >
...
const mapStateToProps = (state) => {
  return {
    recruitmentConductorList: state.apiadd.recruitmentConductorList,
  };
};
const mapDispatchToProps = (dispatch) =>
  bindActionCreators(
    {
      retrieveRecruitmentConuctor,
    },
    dispatch
  );
