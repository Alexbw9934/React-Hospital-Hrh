...
import { retrieveTypeOfCourse } from "../../actions/apiadd";
...
class TypeOfCourse extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      value: {
  ...
  submitData = (e) => {
    ...
    if (Object.keys(errors).length == 0) {
      ...
      let check = this.props.typeofCourseList.map((data) => {
      ...
      if (check.includes(false)) {
        ...
        fetch("http://5.9.111.198:13880/api/TypesofCourses", {
          method: "POST",
          headers: {
  ...
  editForm = (id) => {
    ...
    axios
      .get(`http://5.9.111.198:13880/api/TypesofCourses/${id}`)
      .then((res) => {
  ...
  updateData = (e, id) => {
    ...
    if (Object.keys(errors).length == 0) {
      ...
      fetch(`http://5.9.111.198:13880/api/TypesofCourses/${id}`, {
        method: "PUT",
        headers: {
          Accept: "application/json",
          "Content-Type": "application/json",
        ...
        <div>
          <CRow>
            <CCol xs="12" sm="12">
              <CCard>
                <CCardHeader>
                  <h4>Type of Course List</h4>
                </CCardHeader>
                <CCardBody>
                  <Table
                    dataSource={this.props.typeofCourseList.sort((a, b) =>
                      a.id < b.id ? 1 : -1
                    )}
                    rowKey={"id"}
                  >
...
export default connect(mapStateToProps, mapDispatchToProps)(TypeOfCourse);
