...
import { retrieveTypeOfApproval } from "../../actions/apiadd";
...
class TypeOfApproval extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      value: {
  ...
  componentDidMount() {
    this.props.retrieveTypeOfApproval();
  }
  ...
  submitData = (e) => {
    ...
    if (Object.keys(errors).length === 0) {
      ...
      let check = this.props.typeofApprovalList.map((data) => {
      ...
      if (check.includes(false)) {
        ...
        fetch("http://5.9.111.198:13880/api/TypeofApprovals", {
          method: "POST",
          headers: {
  ...
  editForm = (id) => {
    ...
    axios
      .get(`http://5.9.111.198:13880/api/TypeofApprovals/${id}`)
      .then((res) => {
  ...
  updateData = (e, id) => {
    ...
    if (Object.keys(errors).length === 0) {
      ...
      if (check.includes(false)) {
        ...
        fetch(`http://5.9.111.198:13880/api/TypeofApprovals/${id}`, {
          method: "PUT",
          headers: {
            Accept: "application/json",
            "Content-Type": "application/json",
        ...
        <div>
          <CRow>
            <CCol xs="12" sm="12">
              <CCard>
                <CCardHeader>
                  <h4>Type of Approval List</h4>
                </CCardHeader>
                <CCardBody>
                  <Table
                    dataSource={this.props.typeofApprovalList.sort((a, b) =>
                      a.id < b.id ? 1 : -1
                    )}
                    rowKey={"id"}
                  >
...
const mapStateToProps = (state) => {
  return {
    typeofApprovalList: state.apiadd.typeofApprovalList,
  };
};
const mapDispatchToProps = (dispatch) =>
  bindActionCreators(
    {
      retrieveTypeOfApproval,
    },
    dispatch
  );
export default connect(mapStateToProps, mapDispatchToProps)(TypeOfApproval);
